...
def GetDataFromLikumi(html_content):
    ...
    for link in links:
        try:
            if 'likumi.lv' in link:
                temp = 'https://'+link[link.index('likumi.lv'):]
                if temp not in final_links:
    ...
    for link in links:
        try:
            ...
            if all_paragraphs == None: return None
            text = [t.get_text() for t in all_paragraphs.find_all('p') if 'labojumu_pamats' not in t.get('class')]
...
def MainGetInstructions(folder):
    data = ReadQuestionsFromFolder(folder)
